=== Grit ===

Contributors: Rijo Abraham, Dcrazed.
Requires at least: 4.0
Tested up to: 4.9.4
Stable tag: 1.0.0
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Grit Theme, Copyright 2018 Dcrazed.
Grit is distributed under the terms of the GNU GPL


== Description ==

Grit is a fresh & simple WordPress theme for creative agencies, portfolios, bloggers and is helpful to build any kind of personal, creative, business website.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Frequently Asked Questions ==

= Does this theme support any plugins? =

WooCommerce, Contact Form 7, Jetpack, Yoast SEO are fully supported.

== Changelog ==

= 1.0 - March 28, 2018 =
* Initial release

== Credits ==

Unless otherwise specified, all the theme files, scripts and images are licensed under GNU General Public License.
The exceptions and all known licenses are as follows:

* Images:
	All images are made by the theme author(Rijo Abraham).
	Licensed under CC0 license.

* Animate.css - http://daneden.me/animate:
	Copyright (c) 2014 Daniel Eden
	Licensed under the MIT license - http://opensource.org/licenses/MIT
	
* Bootstrap v3.3.7 (http://getbootstrap.com):
    Copyright 2011-2014 Twitter, Inc.
    Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)
	
* Font Awesome:
	License: SIL OFL 1.1
	URL: http://scripts.sil.org/OFL

* Google Fonts:
	PT Serif (https://fonts.google.com/specimen/PT+Serif)
	License under SIL Open Font License, 1.1
	Copyright © Alexandra Korolkova
	
	Montserrat (https://fonts.google.com/specimen/Montserrat)
	License under SIL Open Font License, 1.1
	Copyright © Julieta Ulanovsky

* Owl Carousel (http://www.owlcarousel.owlgraphic.com/):
	Copyright 2013 Bartosz Wojciechowski
	License under The MIT License (MIT)	

* jQuery v1.11.1  
	Copyright 2005, 2014 jQuery Foundation, Inc.
	License under the MIT license (MIT) - https://jquery.org/license/
	
* Isotope v1.5.25 - http://isotope.metafizzy.co
	Copyright 2012 David DeSandro / Metafizzy
	
* Waypoints - 4.0.1
	Copyright © 2011-2016 Caleb Troughton
	Licensed under the MIT license. - https://github.com/imakewebthings/waypoints/blob/master/licenses.txt
	
* Modernizr v2.8.3 - www.modernizr.com
	Copyright (c) Faruk Ates, Paul Irish, Alex Sexton
	Available under the BSD and MIT licenses: http://www.modernizr.com/license/	
	
* SmoothScroll for websites v1.2.1
	Copyright (c) 2010-2015 Balazs Galambosi
	Licensed under the terms of the MIT license.
	
* WOW - v1.1.3 - 2016-05-06
	Copyright (c) 2016 Matthieu Aussaguel.